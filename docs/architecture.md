...
At most four SDK calls run concurrently under one overall evaluation deadline (`timeoutMs`, default 20 seconds), without retries. Caller cancellation propagates to in-flight calls; cancellation or consent loss prevents new calls. Each response is thresholded independently using the 0.5 decision-confidence and 0.8 hard-violation cutoffs. Attribution retains a separate 0.65 confidence cutoff, with citation aliases validated only against that chunk.
...
Checked unknown tools follow the same configured behavior. Uncertainty and provider unavailability are not compliance judgments. The default is deliberately fail-open for confirmation requests, including unavailable-evaluator fallbacks in both interactive and headless sessions. `confirmationDefault: "deny"` opts into fail-closed automatic resolution. Neither setting overrides explicit policy denials or incomplete-action guards. Existing explicit settings are preserved. Standalone `evaluateSnapshotPolicy` calls that omit confirmation settings also default to automatic acceptance; the OMP runtime passes its settings. Missing credentials remain identified as `login required`, with recovery through `/login typesafe-ai` or `TYPESAFE_API_KEY`.
...
Tool denials return action-bound error text through OMP's `tool_call` result, not detached notifications that could appear beside another parallel call. Opt-in confirmation dialogs and blocked results include the tool, a bounded sanitized/redacted command or path, and the action ID. Code and write bodies are omitted. Assessment text distinguishes uncertainty, explicit model denial, and hard-rule overrides; available decisive rule provenance and `/policy audit` provide recovery context. No alternate-tool bypass is suggested. Direct-action and workflow outcome notifications remain controlled by `showViolationFeedback`; accepted actions stay silent. The `⛨` mark identifies policy output. Defaults are `confirmationThreshold: 1` and `confirmationDefault: approve`: no dialogs, automatic acceptance of prompt decisions. Only explicitly enabled prompts deny without a UI. The status segment distinguishes an active snapshot from a disabled, unavailable, or login-required semantic evaluator.
