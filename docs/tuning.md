...
Any valid chunk denial wins even if another chunk is unavailable. Without a denial, any unavailable chunk makes the aggregate unavailable; otherwise any prompt yields prompt, and allow requires all chunks to allow. Inconsistent resolved models also make the aggregate unavailable unless there is a decisive denial. Conservative provider aggregation leaves explicit host confirmation/fallback opt-ins unchanged: an unavailable aggregate can still follow `confirmationDefault: "approve"` when deliberately configured, but an explicit denial cannot. Separate this host resolution from a provider allow.
