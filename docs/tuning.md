...
The current question is `policy-decision-v5`, compiler is `instruction-compiler-v3`, and thresholds are `policy-thresholds-v2`. The adapter's decision-confidence cutoff is 0.5, attribution cutoff remains 0.65, and hard-violation cutoff remains 0.8. A raw allow at 53% survives confidence adaptation unless hard-violation evidence overrides it. Lower-confidence decisions produce confirmation requests, not fabricated violation findings. Runtime defaults (`confirmationThreshold: 1`, `confirmationDefault: approve`) automatically accept those requests without dialogs, including headless and unavailable-evaluator fallbacks. Scored runs explicitly retain fail-closed automatic settings. Record all stages, not just “Jev denied.” These probabilities are provider evidence, not demonstrated empirical calibration; historical v1 results below are not revalidation of v2.
...
Any valid chunk denial wins even if another chunk is unavailable. Without a denial, any unavailable chunk makes the aggregate unavailable; otherwise any prompt yields prompt, and provider allow requires all chunks to allow. Inconsistent resolved models also make the aggregate unavailable unless there is a decisive denial. Provider aggregation does not override host confirmation settings: an unavailable aggregate follows automatic acceptance by default, while an explicit denial remains blocking. Separate host acceptance from a provider allow.
...
- **Bounded capacity and execution.** Small policies still use one request. Every actual serialized state must fit 40,000 bytes including multi-request partition metadata. The complete plan must fit at most 64 requests before any calls begin. An indivisible rule or repeated action/authorization and dictionary state that cannot fit remains unavailable. At most four SDK calls run concurrently under one overall deadline (`timeoutMs`, default 20 seconds), without retries; caller cancellation propagates and cancellation or consent loss stops new calls. This is not unlimited context capacity or evidence of improved live accuracy.
...
Local or semantic hard denials, incomplete intent, invalid/unavailable evaluation, and provider loss remain ineligible for maintenance approval. A maintenance retry still needs a valid semantic assessment. Keep explicit conservative settings (`confirmationDefault: deny`, `confirmationThreshold: 1`) for scored experiments; runtime defaults now automatically accept confirmation requests with `confirmationDefault: approve`. Automatic acceptance is not maintenance approval and must be recorded separately rather than counted as a provider allow. Do not approve and execute installation/link proposals merely to test this feature; use controlled deterministic coverage of the grant lifecycle.
